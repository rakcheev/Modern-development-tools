        ...
        <div class="prePage">
            ...
            <div class="productBlock clearfix">
                ...
                <section class="right_product clearfix">
                    ...
                    <div class="countBlock clearfix">
                        <div><span>Кол-во:</span></div>
                        <div class="numberBlock clearfix">
                            <a id="numberDown" class="numberDown" href="#"></a>
                            <input class="countInput" type="text" name="countAdd" value="1">
                            <a id="numberUp" class="numberUp" href="#"></a>
                        </div>
                        <span class="onStock">На складе: {{ $knife->count }} шт.</span>
                    </div>
                        <span class="cost_popup">Цена: {{ $knife->price }} р.</span>
                    </div>
                    <button id="to_form" class="button addToCart" onclick="addToCartSerial({{ $knife->id }}); return false;">В корзину</button>
                </div>
            </section>
...
<script type="text/javascript">
        $().ready(function(){
            ...
            $('#numberUp').click(function(){
                var prevNumber = $("input[name=countAdd]").val();
                if (prevNumber > 99) return false;
                var num = +prevNumber + 1;
                $("input[name=countAdd]").val(num);
            });
            $('#numberDown').click(function(){
                var prevNumber = $("input[name=countAdd]").val();
                if (prevNumber <= 1) return false;
                var num = +prevNumber - 1;
                $("input[name=countAdd]").val(num);
            });
            /*Шаблон под фио (запрет цифр и символов кроме - ) + Первая буква заглавная*/
            $("input[name=countAdd]").on("change keyup input click paste", function(){
