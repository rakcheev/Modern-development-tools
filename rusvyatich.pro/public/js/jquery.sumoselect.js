        ...
        var ret = this.each(function () {
            ...
            this.sumo = {
                ...
                createElems: function () {
                    ...
                    if(O.E.attr('disabled') || O.E.hasClass('disabled-select'))
                    ...
                    O.optDiv.append($('<div class="selectItems">'));
                    ...
                    ul = $('<ul class="options">');
                    O.optDiv.find('.selectItems').append(ul);
                ...
                createLi: function (opt,i) {
                    ...
                    if (typeof opt.data('color') != 'undefined')
                    {
                        var $ins = $('<ins>');
                        $ins.css('background-color', opt.data('color'));

                        li.find('label').prepend($ins);
                    }
                    if (typeof opt.data('color-img') != 'undefined')
                    {
                        var $ins = $('<ins class="backsizefix">');
                        $ins.css('background-image', 'url(' + opt.data('color-img') + ')');

                        li.find('label').prepend($ins);
                    }

                    ul = O.optDiv.find('.selectItems').children('ul.options');
                ...
                multiSelelect: function () {
                    ...
                    O.okbtn = $('<p class="btnOk">Готово</p>').click(function () {
                ...
                showOpts: function () {
                    var O = this;
                    if (O.E.attr('disabled') || O.E.hasClass('disabled-select')) return; // if select is disabled then retrun
                    O.is_opened = true;
                    //O.backdrop.show();
                    O.optDiv.addClass('open');
                    O.select.addClass('open-select');
                    O.select.closest('.select-container').addClass('open');
                ...
                hideOpts: function () {
                    ...
                    O.select.removeClass('open-select');
                    O.select.closest('.select-container').removeClass('open');
                    $(document).off('click.sumo');
                    /// for multiple filter
                    if(O.is_multi){
                        O.changeMultipleSelect();
                    }

                    if(O.E.children(':selected').length > 0 && !O.E.children(':selected').hasClass('select-placeholder'))
                    {
                        O.select.closest('.filter-section').addClass('active');
                    }
                    else
                    {
                        O.select.closest('.filter-section').removeClass('active');
                    }

                },
                changeMultipleSelect: function () {
                    if(this.select.find('select').hasClass('group'))
                    {
                        ...
                        $this.find('option').each(function(i, item){
                            ...
                            if(!itemClass && $(item).is(':selected'))
                            {
                                itemClass = $(item).data('class');
                            }

                            $.each(idArray, function(k, input){
                                if(input)
                                {
                                    if($(item).is(':selected') && !$this.closest('form').find('#' + input).is(':checked'))
                                    {
                        ...
                        $this.closest('.filter-section').find('.dimention').removeAttr('class').addClass('dimention ' + itemClass);

                    }
                    else
                    {
                        this.select.find('option').each(function(i, item){
                            if($(item).val())
                            {
                                if($(item).is(':selected') && !$(this).closest('form').find('#' + $(item).val()).is(':checked'))
                                {
                                    $(this).closest('form').find('#' + $(item).val()).trigger('click');
                                }
                                if(!$(item).is(':selected') && $(this).closest('form').find('#' + $(item).val()).is(':checked'))
                                {
                                    $(this).closest('form').find('#' + $(item).val()).trigger('click');
                ...
                setText: function () {
                    ...
                    O.placeholder = "";
                    if (O.is_multi) {
                        sels = O.E.children(':selected').not(':disabled').not('.parent'); //selected options.
                        ...
                        for (i = 0; i < sels.length; i++) {
                            ...
                            else O.placeholder += $.trim($(sels[i]).text()) + ", ";
                        }
                        O.placeholder = $.trim(O.placeholder.replace(/,([^,]*)$/, '$1')); //remove unexpected "," from last.
                    }
                    else {
                        O.placeholder = O.E.children(':selected').not(':disabled').text();
                        if(O.E.children(':selected').hasClass('select-placeholder')){
                            O.select.addClass('placeholder-text');
                        }else{
                           O.select.removeClass('placeholder-text');
                        }
                    }
